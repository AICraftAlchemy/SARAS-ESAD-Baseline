...
class WarmupMultiStepLR(torch.optim.lr_scheduler._LRScheduler):
  ...
  def __init__(self, optimizer, milestones, gammas, last_epoch=-1):
    ...
    assert len(gammas) == len(milestones), 'Milestones and gammas should be of the same length. Gammas are of length ' + str(len(gammas)) + ' and milestones are of length ' + str(len(milestones))
  ...
  def print_lr(self, verbose, i, lr, epoch):

    if verbose:

      print(f"Epoch {epoch}, Step {i}: Learning rate = {lr:.6f}")
